...
from .config import MusicGenConfig, AnalysisResult, Instrument, ArrangementConfig
...
class ScaleGenerator:
    ...
    @staticmethod
    def get_chord(scale_notes: List[int], degree: int, chord_type: str = "triad") -> List[int]:
        """
...
class RhythmGenerator:
    ...
    def generate_basic_rhythm(
        ...
        target_notes = max(1, min(target_notes, beats_per_measure * 4))  # Reasonable limits
    ...
    @staticmethod
    def quantize_to_grid(note_times: List[float], grid_size: float = 0.25) -> List[float]:
        """
    ...
    def generate_melody(
        ...
        for time in rhythm_times:
            if style == "stepwise":
                ...
                current_note_idx = max(0, min(current_note_idx + direction, len(extended_scale) - 1))
            elif style == "arpeggiated":
                # Jump by thirds or fourths
                jump = random.choice([-4, -2, 2, 4])
                current_note_idx = max(0, min(current_note_idx + jump, len(extended_scale) - 1))
            else:  # random
...
def generate_arrangement(
    ...
    rhythm_times = RhythmGenerator.generate_basic_rhythm(
        ...
        arrangement_config.humanization_amount
    )
